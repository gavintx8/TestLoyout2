...
@interface GHotTableView()<UITableViewDataSource, UITableViewDelegate>
@property (nonatomic, copy) void(^scrollCallback)(UIScrollView *scrollView);
@property (nonatomic, strong) NSIndexPath *lastSelectedIndexPath;

@end

@implementation GHotTableView
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        imv.image = [UIImage imageNamed:@"index_banner_01"];
...
- (void)enterGameButtonClick:(UIButton *)button{
    if (self.hotBlock) {
        self.hotBlock(button.tag);
    }
}

- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (self.hotBlock) {
        self.hotBlock(indexPath.row);
    }
}
