...
//    [nextButton setTitle:@"关闭" forState:UIControlStateNormal];
...
//    [nextButton addTarget:self action:@selector(closeButtonClick:) forControlEvents:UIControlEventTouchUpInside];
}
...
- (void)addGestureAndObserver {
    ...
    //添加手势
    [[self currentPhotoView] addGestureRecognizer:longTap];
}
...
-(void)imglongTapClick:(UILongPressGestureRecognizer*)gesture
{
    if(gesture.state==UIGestureRecognizerStateBegan)
    {
        ...
        UIAlertAction *confirm = [UIAlertAction actionWithTitle:@"确认" style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
            NSLog(@"确认保存图片");
            // 保存图片到相册
            UIImageWriteToSavedPhotosAlbum([self currentPhotoView].imageView.image,self,@selector(imageSavedToPhotosAlbum:didFinishSavingWithError:contextInfo:),nil);
        }];
...
- (void)layoutSubviews {
    ...
    CGFloat h = photoScrollH - (KNavBarHeight - 25);
    CGFloat x = 0;
    CGFloat y = KNavBarHeight - 25;
